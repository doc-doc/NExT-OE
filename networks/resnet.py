...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, nb_classes=1000, channel=3):
        self.inplanes = 64
...
def resnet101(pretrained=False, channel= 3, flow=False, **kwargs):

    model = ResNet(Bottleneck, [3, 4, 23, 3], nb_classes=1000, channel=channel, **kwargs)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['resnet101']))
    else:
        model.load_state_dict(torch.load('save_models/resnet101-b121ed2d.pth'))
    return model
...
def resnet152(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        model.load_state_dict(torch.load('save_models/resnet152-b121ed2d.pth'))
    return model


def cross_modality_pretrain(conv1_weight, old_channel,  channel):
    # transform the original 3 channel weight to "channel" channel
    S = 0
    for i in range(old_channel):
        S += conv1_weight[:,i,:,:]
    avg = S/(old_channel*1.0)
...
def weight_transform(model_dict, pretrain_dict, channel):
    ...
    w_ucf = pretrain_dict['conv1_custom.weight']
    old_channel = w_ucf.size(1)
    # print('channel/old channel: {}/{}'.format(channel, old_channel))
    if channel == old_channel:
        wt = w_ucf
    else:
        wt = cross_modality_pretrain(w_ucf, old_channel, channel)
